...
class Bot(Snake):
    ...
    def __strategy_decision(self,board,player,enemies,fx,fy):
        ...
        """ 長さを敵の長さより1大きくに変更する"""
        #かこみこできるか判定
        if (((fx==0) or (fy==0)) and ((fx==self.size-1) or (fy==self.size-1))) and (len(player["body"])>3) and ((fmin<2) or (fmin<4 and self.strategy=="kakomiko")):
            if self.strategy == "kakomiko":
                ...
                self.s_count = 50
        #えるじおできるか判定
        elif ((fx==0) or (fy==0) or (fy==self.size-1) or (fx==self.size-1)) and(len(player["body"])>3) and ((fmin<2) or (fmin<5 and self.strategy=="eruzio")):
